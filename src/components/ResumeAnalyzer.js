...
export default function ResumeAnalyzer({ onViewJobs }) {
  ...
  const atsScore = Number.parseInt(String(result?.ats_score || "0"), 10) || 0;
  const skillsCount = Array.isArray(result?.skills) ? result.skills.length : 0;
  ...
  const analyze = async () => {
    const content =
      mode === "paste"
        ? text.trim()
        : file
          ? `[Resume: ${file.name}] No text extracted - AI will analyze based on filename and general feedback.`
          : "";
    if (!content) {
    ...
    try {
      ...
      if (data.result) {
        ...
        activity.unshift({
          type: "resume",
          score: Number.parseInt(String(data.result.ats_score || "0"), 10) || 0,
      ...
      setError(isNetworkError ? getServerReachabilityHelp(API) : message);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div>
      <div style={{ marginBottom: "22px" }}>
        ...
        <p style={{ fontSize: "14px", color: "#64748b" }}>
          Upload your resume and get instant AI feedback, ATS score, missing keywords, and
          suitable job roles
        </p>
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          ...
          {mode === "upload" ? (
            ...
            >
              ...
              {file ? (
                <>
                  <div style={{ fontSize: "36px", marginBottom: "8px" }}>OK</div>
                ...
                <>
                  <div style={{ fontSize: "44px", marginBottom: "12px" }}>Resume</div>
          ...
          {error && <div style={s.errBox}>Warning: {error}</div>}
          ...
          <button
            onClick={analyze}
            disabled={loading}
            style={{ ...s.analyzeBtn, ...(loading ? { opacity: 0.7, cursor: "not-allowed" } : {}) }}
          >
            {loading ? (
              <>
                <span style={s.spinner} /> Analyzing with Groq AI...
              </>
            ) : (
              "Analyze My Resume"
            )}
          ...
          {!result && !loading && (
            <div style={s.placeholder}>
              ...
              <div style={{ fontSize: "14px", color: "#64748b", lineHeight: 1.6 }}>
                ...
                <br />
                Get: ATS Score - Skills - Missing Keywords
                <br />
                Strengths - Suggestions - Suitable Roles
              </div>
            </div>
          )}
          ...
          {loading && (
            <div style={s.placeholder}>
              ...
              <div style={{ fontSize: "13px", color: "#64748b" }}>
                Reading skills, experience, qualifications...
              </div>
            </div>
          )}
          ...
          {result && !loading && (
            <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: "12px" }}>
                {[
                  ...
                  <div key={label} style={s.scoreCard}>
                    <div style={{ fontSize: "38px", fontWeight: 900, color: scoreColor(val), lineHeight: 1 }}>
                      {val}
                    </div>
                    <div style={{ fontSize: "11px", color: "#64748b" }}>
                      {label === "ATS Score" ? "/100" : "skills"}
                    </div>
                    <div style={{ fontSize: "12px", color: scoreColor(val), fontWeight: 700, margin: "4px 0" }}>
                      {label === "ATS Score" ? scoreLabel(val) : val > 0 ? "Detected" : "Needs More"}
                    </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: "12px" }}>
                ...
                <div style={s.card}>
                  <div style={{ ...s.cardTitle, color: "#fb923c" }}>Weaknesses</div>
              ...
              {result.keywords_missing && result.keywords_missing.length > 0 && (
                <div style={{ ...s.card, borderColor: "rgba(251,146,60,0.25)" }}>
                  <div style={{ ...s.cardTitle, color: "#fb923c" }}>Missing Keywords</div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "7px" }}>
                    {result.keywords_missing.map((sk) => (
                      <span
                        key={sk}
                        style={{
                          fontSize: "12px",
              ...
              <div style={s.card}>
                <div style={s.cardTitle}>Suitable Job Roles</div>
                {(result.job_roles || []).map((job, i) => (
                  <div
                    key={i}
                    style={{
                      display: "flex",
                  ...
                  >
                    <span style={{ fontSize: "13px", color: "#e2e8f0", fontWeight: 600, flex: 1, alignSelf: isMobile ? "stretch" : "auto" }}>
                      {job}
                    </span>
                    <button
                      onClick={() => onViewJobs?.(job)}
                      style={{
                        background: "rgba(30,64,175,0.15)",
