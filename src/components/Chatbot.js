...
I can help you with:
- Government job opportunities (UPSC, SSC, Banking, Railway)
- Private sector career guidance
- Exam preparation tips and syllabus
- Resume and interview advice
- Career path guidance based on your education
...
export default function Chatbot() {
  ...
  const send = async (text) => {
    ...
    try {
      ...
      if (res.status === 401) {
        setMsgs((prev) => [
          ...prev,
          { from: "ai", text: "Session expired. Please log in again.", time: now() },
        ]);
        return;
      }
  ...
  return (
    <div
      style={{
        ...s.root,
        height: isMobile ? "calc(100dvh - 190px)" : isTablet ? "calc(100dvh - 170px)" : s.root.height,
        minHeight: isMobile ? "480px" : s.root.minHeight,
      }}
    >
      <div style={{ ...s.header, flexWrap: isMobile ? "wrap" : "nowrap", gap: isMobile ? "12px" : "0" }}>
        ...
        <button onClick={clear} style={{ ...s.clearBtn, width: isMobile ? "100%" : "auto" }}>
          Clear
        </button>
      </div>
        ...
        {msgs.map((m, i) => (
          ...
          >
            ...
            <div
              style={{
                ...s.bubble,
                maxWidth: isMobile ? "92%" : s.bubble.maxWidth,
                ...(m.from === "user" ? s.userBubble : s.aiBubble),
              }}
            >
              {renderText(m.text)}
            </div>
      ...
      <div style={{ textAlign: "center", fontSize: "11px", color: "#1e293b", padding: "5px" }}>
        Press Enter to send · Groq AI · Free and Fast
      </div>
    </div>
  );
}
...
const s = {
  ...
  avatar: { width: "44px", height: "44px", borderRadius: "12px", background: "linear-gradient(135deg,#1e40af,#3b82f6)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px", fontWeight: 800, color: "#fff" },
